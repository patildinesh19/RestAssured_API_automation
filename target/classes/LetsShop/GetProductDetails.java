...
public class GetProductDetails extends Base {
			...
			for (int i = 0; i < responseBody.getData().size(); i++) {
				for (int j = 0; j < AddProduct.storeproductid.size(); j++)
					if (responseBody.getData().get(i).get_id().equalsIgnoreCase(AddProduct.storeproductid.get(j))) {
						...
						extenttest.log(LogStatus.INFO, "Product Name : " + responseBody.getData().get(i).getProductName()+"added sucessfully");
					}
			}
		} catch (Exception e) {
	...
	{	File Schema = new File(readvalue.get_json_schema_file_for_get_all_product());
		...
		SingletonLogin generator = SingletonLogin.getInstance();
		 ...
		 extenttest.log(LogStatus.INFO,"test case is pass");
	}
	
	@Test
	public void verifyGetProductdetailsFromDB()
	{
		FetchValueFromDB fb = new FetchValueFromDB();
		try {
			
			extenttest = extentreport.startTest("verifyGetProductdetailsFromDB", "Verify Added Product from DB");
			...
			GetProductDetailsPojo responseBody = response.as(GetProductDetailsPojo.class);
				...
				String PRODUCT_NAME=fb.fetchSingleValuefromDB("select PRODUCT_NAME from PRODUCT_DETAILS WHERE PRODUCT_ID=1","PRODUCT_NAME");
				extenttest.log(LogStatus.INFO, "Product name from DB", PRODUCT_NAME);
				assertEquals(responseBody.getData().get(0).getProductName(),PRODUCT_NAME);			
		} catch (IOException e) {
